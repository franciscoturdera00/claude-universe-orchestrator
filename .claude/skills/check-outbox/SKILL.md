---
name: check-outbox
description: Manual umbrella that runs the full sweep + pipeline refresh in sequence and ensures both recurring crons are registered for this session. Most of the time the operator does NOT invoke this — the cron-fired `/sweep` (every 30 min) and `/pipeline` (every 60 min) handle it autonomously, dispatching subagents so Lilo's context stays clean. Use this only when the operator says "/check-outbox", "sweep and refresh now", or on session start to bootstrap both crons.
---
...
- `/sweep` — outbox sweep + relay, every 30 min via the `outbox-sweeper` subagent
...
1. `CronList`. You're looking for two recurring jobs:
   - prompt `/sweep` on schedule `7,37 * * * *`
   - prompt `/pipeline` on schedule `17 * * * *`
2. **Migrate legacy:** if you see a recurring `/check-outbox` job (the old single cron), delete it via `CronDelete` — it has been split. Same for any `/sweep` job on the old `2,12,22,32,42,52 * * * *` cadence.
3. For each missing cron, `CronCreate`:
   - sweep: `cron: "7,37 * * * *"`, `recurring: true`, `prompt: "/sweep"`
